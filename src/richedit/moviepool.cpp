...
public:
	...
	int m_idxupdate;
	QPointer<QTextBrowserEx> m_pTextBrowser;
	int m_vbarpos;
	QSizeF m_docSize;
	int m_movieUpdateInterval;
	int m_docLastVerticalOffset;
public:
	MoviePoolPrivate(QTextDocument *doc)
		: m_pDocument(doc)
	{
		...
		m_pTextBrowser = qobject_cast<QTextBrowserEx*>(vtext.value<QObject*>());
...
MoviePool::MoviePool( QTextDocument *doc )
: QObject(doc)
, d_ptr(new MoviePoolPrivate(doc))
{
	d_ptr->q_ptr = this;
}
...
bool MoviePool::insertMovie( const QString &imageKey, const QString &filePath, bool bForceReplace)
{
	Q_D(MoviePool);
	return d->insertMovie(imageKey, filePath, bForceReplace);
}

bool MoviePool::contain( const QString &imageKey )
{
	Q_D(MoviePool);
	return d->contain(imageKey);
}

void MoviePool::timerEvent( QTimerEvent *e )
{
	Q_D(MoviePool);
	d->timerEvent(e);
}

QImage MoviePool::currentImage( const QString &imageKey ) const
{
	Q_D(const MoviePool);
