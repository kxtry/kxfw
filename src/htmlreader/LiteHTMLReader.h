...
protected:
	...
	virtual void Characters(const std::wstring &rText, DWORD dwAppData, bool &bAbort)
	{
		UNUSED_ALWAYS(rText);
		UNUSED_ALWAYS(dwAppData);
		bAbort = false;
	...
	virtual void Comment(const std::wstring &rComment, DWORD dwAppData, bool &bAbort)
	{
...
public:
	...
	UINT Read(const wchar_t* lpszString);
...
protected:
	...
	virtual bool parseComment(std::wstring &rComment);
	 ...
	 * @author Gurmeet S. Kochar
	 */
	virtual void NormalizeCharacters(std::wstring &rCharacters)
	{
	...
	/**
	 * Moves the buffer back by one wchar_t
	 *
	 * @since 1.0
	 * @author Gurmeet S. Kochar
	 */
	...
	{
		_ASSERT(dwEvent == notifyStartStop  || 
 ...
 * @author Gurmeet S. Kochar
 */
inline bool CLiteHTMLReader::parseComment(std::wstring &rComment)
{
	_ASSERT(m_lpszBuffer != NULL);
	_ASSERT(m_dwBufPos >= 0L);
	_ASSERT(m_dwBufPos + 4 < m_dwBufLen);
	...
	const wchar_t*	lpszBegin = &m_lpszBuffer[m_dwBufPos + 4];	
	// HTML comments end with two hyphen symbols '--'
	const wchar_t*	lpszEnd = ::wcsstr(lpszBegin, L"--");
	...
	{
		rComment = lpszBegin;
		m_dwBufPos += (4 + rComment.size());
		return (true);
	}

	std::wstring	strComment(lpszBegin, lpszEnd - lpszBegin);
	
	// end of buffer?
	if (lpszEnd + (1 * 2) >= m_lpszBuffer + m_dwBufLen)
		return (false);
