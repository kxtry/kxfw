...
UINT CLiteHTMLReader::parseDocument(void)
{
	...
	std::wstring	strCharacters;			// character data 
	std::wstring	strComment;				// comment data
	std::wstring	strT;					// temporary storage
		...
		case L'<':
			{
				...
				if ( (dwCharDataLen) || (strCharacters.size()) )
				{
					strCharacters += std::wstring(&m_lpszBuffer[dwCharDataStart], dwCharDataLen);
					NormalizeCharacters(strCharacters);
					
					if ( (strCharacters.size()) && 
 ...
 */
UINT CLiteHTMLReader::Read(const wchar_t* lpszString)
{
	_ASSERT(lpszString);

	m_dwBufLen = ::wcslen(lpszString);
	if (m_dwBufLen)
	{
...
UINT CLiteHTMLReader::ReadFile(HANDLE hFile)
{
	...
	_ASSERT(::GetFileType(hFile) == FILE_TYPE_DISK);
	...
	lpsz = (const wchar_t*)::MapViewOfFile(hFileMap, FILE_MAP_READ, 0L, 0L, 0L);
	if (lpsz == NULL)
	{
...
LCleanExit:
	if (lpsz != NULL)
		_ASSERT(::UnmapViewOfFile(lpsz));
	if (hFileMap)
		_ASSERT(::CloseHandle(hFileMap));
	return (nRetVal);
}
